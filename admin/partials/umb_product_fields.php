...
?>
<div id="ced_umb_fields" class="panel woocommerce_options_panel">
	<div id="ced_umb_accordian">
		<!-- --------------------------Required fields-------------------------------- -->
		<?php if(count($required_fields)) : ?>
			<div class="ced_umb_panel">
				<div class="ced_umb_panel_heading">
					<h4><?php _e('Walmart Required Fields','ced-umb') ?></h4>
				</div>
				<div class="ced_umb_collapse">
					<div class="options_group ced_umb_label_data">
						<?php 
				...
				if( $isVariableProductCase ) {  // adding variation title field only in case of variation product
					$addedFieldForVariableProTitle = array(
						'type' => '_text_input',
						'id'   => '_umb_variation_title',
						'fields' => array(
							'id'   => '_umb_variation_title',
							'label' => __('Variation Title','ced-umb'),
							'desc_tip' => '',
							'description' => __('This Will Be Your Variation Name In Your walmart.','ced-umb')
							)	
						);
				...
				foreach ($required_fields as $field_array):
					if(isset($field_array['id']) && isset($field_array['type']) && isset($field_array['fields'])){
						...
						if($type=='_umb_select'){
							...
							$optionsHtml = '';
							$optionsHtml .= '<option value="null">'.__('Walmart Subcategory','ced-umb').'</option>';
							if(is_array($options)){
	...
	<?php if(count($framework_fields)) :?>
		<div class="ced_umb_panel">
			<div class="ced_umb_panel_heading">
				<h4><?php _e('Walmart Specific Fields','ced-umb'); ?></h4>
			</div>
			<div class="ced_umb_collapse">
				<?php foreach($framework_fields as $fname=> $ffields_details): ?>
					<?php if(count($ffields_details)) :?>
						<div class="ced_umb_sub_accordion">
							<div class="ced_umb_sub_panel">
								<div class="ced_umb_sub_panel_heading">	
									
								</div>
								<div class="ced_umb_sub_collapse">
									<?php 
	...
	<?php if(count($extra_fields)) :?>
		<div class="ced_umb_panel">
			<div class="ced_umb_panel_heading">
				<h4><?php _e('Walmart Extra Fields','ced-umb') ?></h4>
			</div>
			<div class="ced_umb_collapse">
				<div class="options_group ced_umb_label_data">
					<?php 
							...
							if( $isVariableProductCase ) {
								...
								if(function_exists($efunction_name)) {
									...
									switch($etype){
										...
										<span class="wrap">
											<input id="<?php $id; ?>" placeholder="<?php esc_attr_e( 'Length', 'ced-umb' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="<?php echo $id ?>_length" value="<?php echo esc_attr( wc_format_localized_decimal( get_post_meta( $post->ID, $id.'_length', true ) ) ); ?>" />
											<input placeholder="<?php esc_attr_e( 'Width', 'ced-umb' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="<?php echo $id ?>_width" value="<?php echo esc_attr( wc_format_localized_decimal( get_post_meta( $post->ID, $id.'_width', true ) ) ); ?>" />
											<input placeholder="<?php esc_attr_e( 'Height', 'ced-umb' ); ?>" class="input-text wc_input_decimal last" size="6" type="text" name="<?php echo  $id ?>_height" value="<?php echo esc_attr( wc_format_localized_decimal( get_post_meta( $post->ID, $id.'_height', true ) ) ); ?>" />
										</span>
